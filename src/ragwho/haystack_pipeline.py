...
def retrieval_pipeline_haystack(config):
    ...
    if config["sparse_retriever"] == "BM25":
        document_joiner = DocumentJoiner(
            top_k=config["retrieve_k"], join_mode="reciprocal_rank_fusion"
        )
...
def query_vector_db_once_haystack(retrieval_pipeline: Pipeline, query: str, config):
    if config["sparse_retriever"] == "BM25":
        raw_answer = retrieval_pipeline.run(
            {"text_embedder": {"text": query}, "sparse_retriever": {"query": query}}
        )
    elif config["sparse_retriever"].lower() == "none":
        raw_answer = retrieval_pipeline.run({"text_embedder": {"text": query}})
    else:
        raise ValueError(
            'Invalid sparse_retriever provided in config. Use "BM25" or "None".'
        )

    processed_answer = {
        "question": query,
        "answers": [
            {"text": doc.content, config["distance_type"].lower(): doc.score}
...
def query_vector_db_list_haystack(
    ...
    answer_list = [
        query_vector_db_once_haystack(retrieval_pipeline, q, config) for q in query_list
    ]

    return answer_list


def rag_pipeline_haystack(config, api_key_variable: str = "OPENAI_API_KEY"):
    vector_db, embedder = setup_vector_db(
        encoder_name=config["encoder_name"],
        client_source=config["client_source"],
        input_folder=config["input_text_folder"],
...
def rag_query_list_haystack(rag_pipeline: Pipeline, queries: list[str], config):
...
if __name__ == "__main__":
    ...
    result = query_vector_db_once_haystack(pipeline, question, config[args.config_name])
    print(result)
