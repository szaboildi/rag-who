...
from ragwho.haystack_pipeline import (
    retrieval_pipeline_haystack,
    query_vector_db_list_haystack,
)
...
def eval_recall_sentence(
    ...
    for i in range(len(results)):
        ...
        recall = sum(
            [
                any([corr_a in r for r in retreived_passages])
                for corr_a in correct_answers_sent[i]["answers"]
            ]
...
def eval_recall_passage(
    ...
    for i in range(len(results)):
        ...
        not_found_answers_i = [
            corr_a
            for corr_a in correct_answers_passages[i]["answer_passages"]
            if corr_a not in retreived_passages
        ]
...
def ndcg_manual(returned_rel: list[int], ideal_rel: list[int]):
    dc_gain = 0
    idc_gain = 0
...
def ndcg_scorer_manual(
    ...
    correct_answers_sentences: list[dict[str, str]],
    correct_answers_passage_rel: list[dict[str, str]],
):
    # dcg_scores = []
    ndcg_scores = []
    for i in range(len(results)):
        ...
        y_returned_relevance = [
            a["is_relevant"]
            for a in is_relevant_sentence_dict(
                results[i], correct_answers_sentences[i]
            )["answers"]
        ]
        ...
        ndcg, dcg, idcg = ndcg_manual(
            y_returned_relevance, y_ideal_relevance[:n_passages_retrieved]
        )
        # print(f"relevance scores: {y_returned_relevance}\n" +
...
def retrieve_and_eval(config_name: str = "default", mode="qdrant"):
    ...
    with open(
        os.path.join(
            ...
            config[config_name]["sample_qa_file"],
        )
    ) as f:
    ...
    if mode == "qdrant":
        client, encoder = setup_vector_db(
            ...
            relevance_score_file_prefix=config[config_name][
                "relevance_score_file_prefix"
            ],
    ...
    with open(
        os.path.join(
            ...
            + f"{config[config_name]['chunk_overlap']}.json",
        )
    ) as f:
