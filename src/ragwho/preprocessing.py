import os
import json
from ragwho.utils import is_relevant_sentence_str
...
def process_text(
    ...
    return_format: str = "ls",
    force_write_qa_passages: bool = False,
    ...
    json_out_location = os.path.join(
        input_folder_qa,
        f"{relevance_score_file_prefix}_length{length}_overlap{words_overlap}.json",
    )
    if force_write_qa_passages or not os.path.exists(json_out_location):
        print(
            "Relevance scores don't exist for these chunks or rewrite was forced, calculating them now"
        )
        corr_answer_ls_dict = []
